  ...
  useEffect(() => {
    ...
    fetch(url)
      .then((response) => {
        ...
        throw new Error("Unable to fetch the book details.");
      })
      .then((res) => {
            ...
            <div className="container ">
              <div className="row mt-2">
                ...
                <div className="col-9 mt-3">
                    ...
                    <p>
                        <em>Subjects: {
                            book.subjects.substr(0, book.subjects.length - 1).substr(1).split(',').map((val, idx) => {
                                return(   
                                    <span key={idx}>{val.substr(0, val.length - 1).substr(1).replace('"', ', ').replace('--', '–––')}</span>
                                )
                            })
                        }</em>
                    </p>
                  ...
                  <Link to="/" className="btn btn-lg btn-danger mt-3 mb-3">
                    Back to books
                  </Link>
                </div>
              </div>   
